...
extension UIImage {
    ...
    // Work with pixels
    @nonobjc public func resizeWhileMaintainingAspectRatioToSize(
        to size: CGSize,
        scale: CGFloat = 1
    ) -> UIImage {
        let newSize: CGSize
        
        let widthRatio  = size.width / self.pixelSize.width
        let heightRatio = size.height / self.pixelSize.height
        
        if widthRatio > heightRatio {
            newSize = CGSize(
                width: floor(self.pixelSize.width * widthRatio),
                height: floor(self.pixelSize.height * widthRatio)
            )
        } else {
            newSize = CGSize(
                width: floor(self.pixelSize.width * heightRatio),
                height: floor(self.pixelSize.height * heightRatio)
            )
        }
